...
class UserModelCase(unittest.TestCase):
    ...
    def test_password_hashing(self):
        u = User(username='susan', email='s@s.s')
        u.set_password('cat')
    ...
    def test_follow(self):
        ...
        u2 = User(username='Susan', email='s@s.s')
        db.session.add(u1)
        db.session.add(u2)
    ...
    def test_follow_posts(self):
        u1 = User(username='john', email='j@j.j')
        u2 = User(username='susan', email='s@s.s')
        u3 = User(username='mary', email='m@m.m')
        u4 = User(username='david', email='d@d.d')
        db.session.add_all([u1, u2, u3, u4])

        now = datetime.utcnow()
        p1 = Post(body="post from john", author=u1, timestamp=now + timedelta(seconds=1))
        p2 = Post(body="post from susan", author=u2, timestamp=now + timedelta(seconds=14))
